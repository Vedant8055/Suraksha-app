...
class CyberProtectionService {
  ...
  Future<CyberReportResult> submitReport({
    ...
    if (normalized.isEmpty) {
      throw ArgumentError('At least one report category is required.');
    }
    final response = await _dio.post(
  ...
  Future<CyberReportListItem> savePortalAcknowledgement({
    required String reportId,
    required String acknowledgementNumber,
  }) async {
    final response = await _dio.patch(
      ApiConstants.cyberReportPortalAcknowledgement(reportId),
      data: {'portalAcknowledgementNumber': acknowledgementNumber.trim()},
    );
    return CyberReportListItem.fromJson(
      Map<String, dynamic>.from(response.data as Map),
    );
  }
