...
class RouteSafetyState {
  ...
  const RouteSafetyState({
    ...
    this.statusMessage = 'statusLearningDailyRoute',
...
class RouteSafetyNotifier extends StateNotifier<RouteSafetyState> {
  ...
  Future<void> setActiveMapRoute({
    ...
    state = state.copyWith(
      ...
      statusMessage: _activeMapRoute.isEmpty
          ? state.statusMessage
          : 'statusMonitoringMapRouteTo|$destinationName',
    );
    await refreshNow();
  }
  ...
  Future<void> clearActiveMapRoute() async {
    ...
    state = state.copyWith(
      ...
      statusMessage: 'statusMapRouteCleared',
    );
    await refreshNow();
  }
  ...
  Future<void> markUserSafe() async {
    ...
    state = state.copyWith(
      ...
      statusMessage: 'statusSafetyConfirmed',
    );
  }
  ...
  Future<void> resetLearnedRoute() async {
    ...
    state = const RouteSafetyState(
      statusMessage: 'statusRouteHistoryReset',
    );
  }
  ...
  Future<void> _handleMonitorUpdate(SafetyMonitorState monitor) async {
    ...
    if (monitoringMapRoute) {
      baseRiskFactors.add('routeFactorSelectedMapRoute');
    } else if (matchedRoutine != null) {
      baseRiskFactors.add(
        matchedRoutine.profile.tripCount >= _minTripsForStrongRoutine
            ? 'routeFactorStrongCommute'
            : 'routeFactorRepeatedDaily',
      );
    } else if (_routineProfiles.isNotEmpty) {
      baseRiskFactors.add('routeFactorNoMatchingRoutine');
    }
    ...
    final statusMessage = monitoringMapRoute
        ? assessment.deviatedFromLearnedRoute
              ...
              ? 'statusFollowingMapRoute'
              : 'statusFollowingMapRouteTo|$destinationName'
        : matchedRoutine == null
  ...
  String? _buildLearningProgressLabel({
    ...
    if (matchedRoutine != null) {
      ...
      return trips >= _minTripsForStrongRoutine
          ? 'routeProgressStrongCommute|$trips'
          : 'routeProgressProvisional|$trips|$_minTripsForStrongRoutine';
    }
    if (_routineProfiles.isEmpty) {
      final need = _minTripPoints - _currentTripPoints.length;
      if (_currentTripPoints.isNotEmpty && need > 0) {
        return 'routeProgressRecordingTrip|$need';
      }
      if (_learnedTrips.isEmpty) {
        return 'routeProgressTakeUsualRoute';
      }
      return 'routeProgressSavedTrips|${_learnedTrips.length}';
    }
    return 'routeProgressRoutinesWaiting|${_routineProfiles.length}';
  }
  ...
  void _beginSafetyCheck(Position position) {
    ...
    unawaited(() async {
      ...
      await LocalAlertService.instance.showRouteDeviationAlert(
        title: l10n.t('routeChangedNotificationTitle'),
        body: l10n
            .t('routeChangedNotificationBody')
            .replaceAll('{seconds}', '$defaultSafetyCheckSeconds'),
      );
    }());
    state = state.copyWith(
      ...
      statusMessage: 'statusSafeRouteChanged',
      lastPosition: position,
    );
