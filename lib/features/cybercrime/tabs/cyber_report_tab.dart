...
class _CyberReportTabState extends State<CyberReportTab> {
  ...
  Future<void> _loadMyReports() async {
    ...
    try {
      ...
      if (await showCyberAuthExpiredBanner(
        context,
        error,
        requestPath: '/cybercrime/my-reports',
      )) {
        return;
      }
      if (!mounted) return;
  ...
  Future<void> _linkVaultEvidence(String reportId) async {
    for (final id in _vaultEvidenceIds) {
      await widget.service.linkEvidenceToReport(
        evidenceId: id,
        reportId: reportId,
      );
    }
  ...
  Future<void> _pickFromVault() async {
    final l10n = AppLocalizations.of(context);
    try {
      final items = await widget.service.listEvidence(linked: 'false');
      if (!mounted) return;
      if (items.isEmpty) {
  ...
  Future<void> _submit({required bool draft}) async {
    ...
    try {
      ...
      if (!mounted) return;
      final baseMessage = draft
          ? l10n.t('draftSavedOnline')
          : l10n.t('reportGenerated');
      var evidenceMessage = '';
            ...
            ..._myReports
                ...
                .map(
                  (report) => ListTile(
                    ...
                    title: Text(
                      formatReportCategories(context, report),
                      maxLines: 1,
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Wrap(
                          ...
                          runSpacing: 4,
                          children: [
                            CyberReportStatusChip(report: report, compact: true),
                            if (reportFiledOnPortal(report))
                              Container(
      ...
      children: [
        ...
        if (report != null &&
          ...
          CyberPortalFilingCard(
            ...
            service: widget.service,
            onApiError: widget.onApiError,
            complaintText: report.firStyleReport,
            pdfReport: report,
            onAcknowledgementSaved: _loadMyReports,
          ),
        ],
      ],
    );
...
class _VaultEvidencePickerDialogState
    extends State<_VaultEvidencePickerDialog> {
          ...
          children: widget.items
              .map(
                (item) => CheckboxListTile(
                  ...
                  subtitle: Text(
                    localizedEvidenceCategory(context, item.category),
                  ),
                ),
              )
              .toList(),
