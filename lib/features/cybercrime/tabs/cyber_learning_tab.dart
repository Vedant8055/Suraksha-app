...
import 'package:suraksha_women_safety_app/features/cybercrime/widgets/cyber_deepfake_emergency_panel.dart';
import 'package:suraksha_women_safety_app/localization/app_localizations.dart';
import 'package:suraksha_women_safety_app/theme/app_theme.dart';

/// Cyber Crime Law Library with merged deepfake emergency guidance.
...
class _CyberLearningTabState extends State<CyberLearningTab> {
  DeepfakeResources? _deepfakeResources;

  @override
  void initState() {
    super.initState();
    unawaited(_loadDeepfakeResources());
  }

  Future<void> _loadDeepfakeResources() async {
    try {
      final resources = await widget.service.getDeepfakeResources();
      if (mounted) setState(() => _deepfakeResources = resources);
    } catch (_) {}
  }

  void _openDeepfakeLawGuide() {
    final topic = deepfakeLawTopic();
  ...
  Widget build(BuildContext context) {
    ...
    final deepfakeResources =
        _deepfakeResources ?? fallbackDeepfakeResourcesFor(l10n);
    ...
    return CustomScrollView(
      ...
      slivers: [
        ...
        SliverToBoxAdapter(
          child: CyberDeepfakeEmergencyPanel(
            resources: deepfakeResources,
            onOpenFullGuide: _openDeepfakeLawGuide,
          ),
        ),
        // ── Topic Grid ───────────────────────────────────
        SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                Wrap(
                  spacing: 6,
                  runSpacing: 6,
                  children: [_Badge(label: content.short, color: topic.color)],
                ),
